...
safe_download_geo_file <- function(url, dest, max_retries = 3) {
  ...
  if (file.exists(dest) && !is_gz_file_ok(dest)) {
    message("Incomplete or corrupted file found; deleting and downloading again: ", basename(dest))
    unlink(dest, force = TRUE)
  }

  for (attempt in seq_len(max_retries)) {
    if (file.exists(dest) && is_gz_file_ok(dest)) {
      message("Local file already exists; skipping download: ", basename(dest), " | ", round(file.info(dest)$size / 1024 / 1024, 1), " MB")
      return(TRUE)
    }
    ...
    for (method in method_order) {
      message("Download attempt ", attempt, "/", max_retries, " [", method, "]: ", basename(dest))
      ...
      ok <- tryCatch({
        ...
        message("Download failed [", method, "]: ", conditionMessage(e))
        FALSE
      })
      ...
      if (ok && local_ok) {
        message("Download completed: ", basename(dest), " | ", round(file.info(dest)$size / 1024 / 1024, 1), " MB")
        return(TRUE)
      }

      if (file.exists(dest)) {
        message("Download is incomplete or unreadable; deleting the partial file: ", basename(dest), " | ", round(file.info(dest)$size / 1024 / 1024, 1), " MB")
        unlink(dest, force = TRUE)
      }
    }
  ...
  stop("download failed after retries: ", url,
       "\nPlease download this file manually with a browser to: ", normalizePath(dirname(dest), winslash = "/", mustWork = FALSE),
       "\nThe file name must remain: ", basename(dest))
}
...
get_series_matrix_expr <- function(gse_id) {
  ...
  if (ncol(feature) > 0 && "gene_assignment" %in% colnames(feature)) {
    ...
    message("gene_assignment parsing quality: valid=", q$n_valid,
            ", unique=", q$n_unique,
  ...
  if ((is.null(gene_symbol) || length(gene_symbol) != nrow(expr) || all(is.na(gene_symbol))) && ncol(feature) > 0) {
    ...
    if (length(symbol_candidates) > 0) {
      for (cc in symbol_candidates) {
        ...
        q <- symbol_quality(gene_symbol_try, nrow(expr))
        message("Feature symbol candidate-column check: ", cc,
                " | valid=", q$n_valid,
                ", unique=", q$n_unique,
                ", duplicate=", q$duplicate_n,
                ", symbol_like=", round(q$symbol_like_prop, 3),
  ...
  if (is.null(gene_symbol) || length(gene_symbol) != nrow(expr) || all(is.na(gene_symbol))) {
    ...
    if (!is.null(gpl_id) && gpl_id != "") {
      message("No reliable symbol in the Series Matrix; trying GPL annotation:", gpl_id)
      gene_symbol_try <- get_symbol_from_gpl(gpl_id, expr_ids)
      q <- symbol_quality(gene_symbol_try, nrow(expr))
      message("GPL symbol parsing quality: valid=", q$n_valid,
              ", unique=", q$n_unique,
              ", duplicate=", q$duplicate_n,
              ", symbol_like=", round(q$symbol_like_prop, 3),
              ", numeric=", round(q$numeric_prop, 3),
  ...
  if (is.null(gene_symbol) || length(gene_symbol) != nrow(expr) || all(is.na(gene_symbol))) {
    stop(paste0(
      "The Series Matrix/GPL could not provide reliable gene symbols.\n",
      "Current fData column names:\n", paste(colnames(feature), collapse = ", "),
      "\nExpression-matrix row-name examples:\n", paste(head(expr_ids, 20), collapse = ", "),
      "\nSuggestion: inspect the GPL annotation columns or use probe-level/no-merge mode."
    ))
  }
  ...
  list(
    ...
    expr_detect = det,
    msg = paste0(
      "Using the GEO Series Matrix expression matrix.\n",
      "Symbol parsing method:", symbol_col, "\n",
      ...
      "Automatically detected data type:", det$type, "\n",
      "Recommended differential-analysis method:", det$method, "\n",
      "Duplicate-Symbol aggregation method:", attr(expr_df, "collapse_method"), "\n",
      "Detection reason:", det$reason,
      warning_msg
    )
  )
}
...
read_table_any <- function(f) {
  tryCatch({
    ...
    message("Read failed: ", basename(f), " | ", e$message)
    NULL
  })
}
...
read_one_single_sample_expr_file <- function(f) {
  dat <- tryCatch({
    ...
    message("Failed to read single-sample file: ", basename(f), " | ", e$message)
    NULL
  })
...
try_ultra_fast_expression_matrix <- function(f) {
  ...
  message("Ultra-fast matrix reader: ", bn)
  ...
  dt <- tryCatch({
    ...
    message("Ultra-fast matrix reader failed: ", bn, " | ", e$message)
    NULL
  })
...
try_fast_submitted_expression_matrix <- function(f) {
  ...
  message("Fast matrix reader: ", bn)

  dat <- tryCatch({
    data.table::fread(f, data.table = FALSE, fill = TRUE, check.names = FALSE)
  }, error = function(e) {
    message("Fast matrix reader failed: ", bn, " | ", e$message)
    NULL
  })
...
report_scRNA_files <- function(supp_dir, gse_id = "") {
  ...
  if (length(sc_files) > 0) {
    message(
      "Single-cell/10X-related files detected; they will be skipped in bulk analysis:",
      length(sc_files), " files. Examples:",
...
stop_if_only_scRNA_no_bulk_candidates <- function(supp_dir, candidate_files, gse_id = "") {
  if (length(candidate_files) == 0 && is_10x_single_cell_dir(supp_dir)) {
    ...
    stop(paste0(
      "10X/single-cell RNA-seq data detected:", gse_id, "\n",
      "No expression matrix or per-sample bulk expression/count files suitable for bulk RNA-seq or microarray analysis were found.\n",
      "Single-cell files such as matrix.mtx, barcodes.tsv, features.tsv/genes.tsv, and hashing_info were skipped.\n",
      "Single-cell file examples:\n",
      paste(sc_files, collapse = "\n"), "\n",
      "For single-cell analysis, use a dedicated Seurat or Scanpy workflow."
    ))
  }
  invisible(FALSE)
}
...
debug_supplementary_files_message <- function(supp_dir, max_show = 80) {
  ...
  if (length(files) == 0) {
    return("No files were found after extraction.")
  }
  ...
  paste0(
    "Files found after extraction:", length(files), "\n",
    "Preview of the largest/most relevant files:\n",
...
filter_single_files_by_metadata_cohort <- function(files, series_meta, min_cohort_fraction = 0.30) {
  ...
  if (best_sig != "unknown" && best_n >= 2 && best_n / total_n >= min_cohort_fraction) {
    ...
    message("Multiple GSM per-sample cohorts detected; keeping the largest metadata-compatible cohort:", best_sig,
            "; kept ", length(kept), " files; skipped ", length(skipped), " files.")
    message("Skipped cohort summary:", paste(head(names(tab)[names(tab) != best_sig], 10), collapse = ", "))
    return(kept)
  }
...
diagnose_failed_supplementary_files <- function(files, max_show = 25) {
  if (length(files) == 0) {
    return("No supplementary candidate files were available for inspection.")
  }
  ...
  paste0(
    ...
    paste0(names(tab), ": ", as.integer(tab), " files", collapse = "\n"),
    "\n\nRepresentative file preview:\n",
    paste0(show$file, " | ", show$class, " | ", show$size_KB, " KB | ", show$preview, collapse = "\n"),
    if (nrow(info) > max_show) paste0("\n... Total ", nrow(info), " candidate files; showing the first ", max_show, " files") else "",
    "\n\nNo gene-level count/FPKM/TPM expression matrix directly usable by DESeq2/limma was detected.",
    "\nIf the files are mainly alignment/mapping statistics or SRA metadata/stubs, GEO did not provide a processed expression matrix. Re-quantify from SRA FASTQ/BAM or use a GEO series with a processed matrix."
  )
}

summarize_rejected_candidate_files <- function(files, max_show = 30) {
  if (length(files) == 0) return("No candidate files.")
  info <- data.frame(
    file = safe_utf8(basename(files)),
    size_KB = round(suppressWarnings(file.info(files)$size) / 1024, 2),
    preview = vapply(files, function(ff) paste(safe_read_preview_lines(ff, n = 2), collapse = " | "), character(1)),
  ...
  paste0(
    ...
    if (nrow(info) > max_show) paste0("\n... Total ", nrow(info), " candidate files") else ""
  )
}
...
merge_single_sample_expr_files_robust <- function(files, max_files = 5000) {
  ...
  if (length(tiny_files) > 0) {
    ...
    message("Skipping files that are too small to be gene-level expression matrices:", length(tiny_files), " files")
    message("Small-file categories:", paste0(names(tiny_classes), "=", as.integer(tiny_classes), collapse = "; "))
    message("Small-file examples:", paste(head(basename(tiny_files), 10), collapse = ", "))
  }
  ...
  message("Per-sample files recognized:", length(count_list), "; failed/skipped:", failed, "; duplicate Symbols were aggregated within each file")
  if (length(count_list) > 0) {
    message("Recognized sample examples:", paste(head(names(count_list), 10), collapse = ", "))
  }

  if (length(count_list) < 2) return(NULL)

  message("Merging per-sample matrices:", length(count_list), " samples")
  ...
  message("Per-sample matrix merge completed:", nrow(raw), " genes x ", ncol(raw) - 1, " samples")
...
try_submitter_rpm_excel_matrix <- function(f) {
  ...
  message("Submitter-normalized Excel candidate reader: ", bn)
  ...
  if (length(best_msg) > 0) {
    message("The Excel parser did not accept this file. Most recent reasons:", paste(tail(best_msg, 5), collapse = " | "))
  } else {
    message("The Excel parser did not accept this file.")
  }
  NULL
}
...
try_plain_gene_sample_text_matrix <- function(f) {
  ...
  message("Robust large TXT/TSV matrix reader: ", bn)
  ...
  dat <- tryCatch({
    ...
    message("Robust TXT/TSV reader failed: ", bn, " | ", e$message)
    NULL
  })
...
parse_expression_candidate_file <- function(f) {
  ...
  if (is_obvious_single_sample_expression_file(f)) {
    message("Skipping the gene-by-sample matrix parser; passing this file to the per-sample merger: ", basename(f))
    return(NULL)
  }

  message("Trying candidate expression-matrix reader: ", basename(f))
...
load_method_preferred_expression_source <- function(gse_id, method_preference = "auto") {
  ...
  message("Method-aware data-source candidate order (", method_preference, "):")
  ...
  for (f in files) {
    ...
    if (method_preference == "limma" && identical(det$type, "raw_count")) {
      message("Force limma prefers normalized/TPM/FPKM data; skipping raw-count candidate: ", basename(f))
      next
    }
    if (method_preference == "DESeq2" && !identical(det$type, "raw_count")) {
      message("Force DESeq2 accepts raw counts only; skipping non-count candidate: ", basename(f))
      next
    }
...
download_geo_supplementary_processed_first <- function(gse_id, supp_dir, allow_raw_fallback = TRUE, method_preference = "auto") {
  ...
  if (nrow(remote) == 0 && nrow(ncbi_generated) == 0) {
    message("Could not pre-scan the supplementary/NCBI-generated list; falling back to getGEOSuppFiles.")
  ...
  if (nrow(processed_all) > 0) {
    ...
    message("Organized gene-by-sample candidate matrices found; using the method-aware processed-first strategy and postponing RAW.tar download/extraction:")
    ...
    if (nrow(other) > 0) {
      ...
      if (nrow(meta_like) > 0) {
        message("Also downloading metadata/clinical/sample files:")
  ...
  if (allow_raw_fallback && nrow(raw_archives) > 0) {
    message("No organized gene-by-sample matrix was found; downloading RAW/archive as a fallback:")
  ...
  if (nrow(other) > 0) {
    message("No processed matrix or RAW archive was found; downloading other supplementary files for parsing.")
    for (i in seq_len(nrow(other))) {
      ...
      if (grepl("hhs\\.gov|vulnerability|disclaimer|privacy|accessibility|faq|help|login|mailto:",
                u, ignore.case = TRUE)) {
        message("Skipping non-GEO web link: ", u)
        next
      }
      ...
      if (grepl("^https?://", f, ignore.case = TRUE) &&
        ...
        message("Skipping non-GEO web file name: ", f)
        next
      }
      ...
      if (!file.exists(dest)) {
        tryCatch({
          ...
          message("Skipping link after download failure: ", u, " | ", conditionMessage(e))
        })
      }
    }
    return(invisible(TRUE))
...
merge_expression_source_objects <- function(obj1, obj2, meta_total_n = NA_integer_, note = "") {
  ...
  obj1$msg <- gsub("Samples loaded:[^\n]+", paste0("Samples loaded:", nmerged), obj1$msg)
  obj1$msg <- gsub("Genes:[^\n]+", paste0("Genes:", nrow(merged)), obj1$msg)
...
make_supplementary_result_object <- function(raw, meta, expr_file, meta_file, metadata_source,
  ...
  list(
    ...
    msg = paste0(
      "The Series Matrix/GPL had no symbols; a supplementary gene-by-sample matrix was selected automatically.\n",
      "Samples loaded:", ncol(raw) - 1, "\n",
      "Genes:", nrow(raw), "\n",
      ...
      "Symbol selection reason:", symbol_col_reason, "\n",
      "Reader mode:", reader_mode, "\n",
      "Automatically detected data type:", expr_detect$type, "\n",
      "Recommended differential-analysis method:", expr_detect$method, "\n",
      "Duplicate-Symbol aggregation method:", attr(raw, "collapse_method"), "\n",
      "Gene ID type:", ifelse(is.null(attr(raw, "gene_id_type")), NA, attr(raw, "gene_id_type")), "\n",
      "Gene ID conversion:", ifelse(is.null(attr(raw, "gene_id_conversion_reason")), NA, attr(raw, "gene_id_conversion_reason")), "\n",
      "Gene IDs converted:", ifelse(is.null(attr(raw, "gene_id_converted_n")), NA, attr(raw, "gene_id_converted_n")), "\n",
      "Detection reason:", expr_detect$reason,
      extra_msg
    )
  )
}
...
download_geo_raw_archives_only <- function(gse_id, supp_dir) {
  ...
  if (is.null(remote) || nrow(remote) == 0) {
    message("RAW/archive fallback: could not pre-scan the supplementary list.")
    return(invisible(FALSE))
  }

  raw_archives <- remote[is_raw_archive_filename(remote$file), , drop = FALSE]
  if (nrow(raw_archives) == 0) {
    message("RAW/archive fallback: no RAW/archive files were found.")
    return(invisible(FALSE))
  }

  message("Processed-matrix parsing failed; starting RAW/archive fallback download:")
  message(paste(raw_archives$file, collapse = "\n"))
  ...
  for (i in seq_len(nrow(raw_archives))) {
    ...
    if (!file.exists(dest)) {
      tryCatch({
        ...
        message("RAW/archive fallback download failed: ", u, " | ", conditionMessage(e))
      })
    }
  }
  invisible(TRUE)
...
get_supplementary_expr <- function(gse_id, force_raw_fallback = FALSE) {
  ...
  message("Starting supplementary expression-matrix parsing...")
  message("Pre-scanning supplementary files: ", gse_id)
  ...
  if (length(local_processed) > 0 && !isTRUE(force_raw_fallback)) {
    message("Local processed expression files detected; RAW/archive extraction is deferred and will run automatically if processed parsing fails:")
  ...
  if (length(archives) > 0) {
    for (af in archives) {
      message("Extracting archive: ", basename(af))
      if (grepl("\\.zip$", af, ignore.case = TRUE)) {
        try(utils::unzip(af, exdir = dirname(af)), silent = TRUE)
      } else {
        try(utils::untar(af, exdir = dirname(af)), silent = TRUE)
  ...
  if (length(candidate_files) == 0) {
    stop(paste0(
      "Supplementary files were downloaded and extracted, but no recognizable expression file was found.\n\n",
...
"))
  ...
  for (f in candidate_files) {
    ...
    message("Trying organized supplementary matrix file: ", basename(f))

    res <- tryCatch(parse_expression_candidate_file(f), error = function(e) {
      message("Candidate matrix parsing failed; skipping this file and continuing: ", basename(f), " | ", conditionMessage(e))
      NULL
    })
...
Source selection note: processed matrix ", basename(f),
      ...
      return(list(
        ...
        msg = paste0(
          ...
          "Symbol column:", res$symbol_col, "\n",
          "Gene ID column:", ifelse(is.null(res$gene_id_col), NA, res$gene_id_col), "\n",
          "Symbol selection reason:", ifelse(is.null(res$symbol_col_reason), NA, res$symbol_col_reason), "\n",
          "Reader mode:", ifelse(isTRUE(res$ultra_fast_reader), "ultra-fast submitted expression matrix", ifelse(isTRUE(res$fast_reader), "fast submitted expression matrix", "generic parser")), "\n",
          "Automatically detected data type:", det$type, "\n",
          "Recommended differential-analysis method:", det$method, "\n",
          "Duplicate-Symbol aggregation method:", attr(res$raw, "collapse_method"), "\n",
          "Gene ID type:", ifelse(is.null(attr(res$raw, "gene_id_type")), ifelse(is.null(res$gene_id_type), NA, res$gene_id_type), attr(res$raw, "gene_id_type")), "\n",
          "Gene ID conversion:", ifelse(is.null(attr(res$raw, "gene_id_conversion_reason")), ifelse(is.null(res$gene_id_conversion_reason), NA, res$gene_id_conversion_reason), attr(res$raw, "gene_id_conversion_reason")), "\n",
          "Gene IDs converted:", ifelse(is.null(attr(res$raw, "gene_id_converted_n")), ifelse(is.null(res$gene_id_converted_n), NA, res$gene_id_converted_n), attr(res$raw, "gene_id_converted_n")), "\n",
          "Detection reason:", det$reason
        )
      ))
    }
  ...
  message("Candidate per-sample expression/count files after deduplication and structural filtering:", length(single_files))
  if (length(single_files) > 0) {
    message("Candidate per-sample file examples:", paste(head(basename(single_files), 10), collapse = ", "))
  }
  ...
  if (!is.null(tenx_res)) {
    ...
    return(list(
      ...
      expr_detect = det,
      msg = paste0(
        "The Series Matrix/GPL had no symbols; the supplementary 10X matrix.mtx expression matrix was read automatically.\n",
        "Samples loaded:", ncol(raw) - 1, "\n",
        "Genes:", nrow(raw), "\n",
        "Metadata file: ", meta_file_used, "\n",
        "Metadata source: ", meta_source, "\n",
  ...
  if (length(single_files) >= 2) {
    message("Possible per-sample expression/count files detected; starting automatic merge...")
    ...
    if (!is.null(raw)) {
      ...
      return(list(
        ...
        expr_detect = det,
        msg = paste0(
          "The Series Matrix/GPL had no symbols; supplementary per-sample expression/count files were merged automatically.\n",
          "Samples loaded:", ncol(raw) - 1, "\n",
          "Genes:", nrow(raw), "\n",
          "Metadata file: ", meta_file_used, "\n",
          "Metadata source: ", meta_source, "\n",
  ...
  if (!isTRUE(force_raw_fallback)) {
    message("Processed/organized supplementary files were not accepted; preparing the RAW/archive fallback.")
    download_geo_raw_archives_only(gse_id, supp_dir)
    return(get_supplementary_expr(gse_id, force_raw_fallback = TRUE))
  }

  stop(paste0(
    "Supplementary files were found, but no suitable gene-by-sample expression matrix was recognized.
",
    "Processed matrices and the RAW/archive fallback were tried. The current fast bulk workflow does not directly parse CEL/FASTQ/BAM files or mapping-statistics-only archives.
...
load_geo_expr_auto <- function(gse_id, use_cache = TRUE, rebuild_cache = FALSE) {
  ...
  if (isTRUE(use_cache) && !isTRUE(rebuild_cache) && file.exists(cf)) {
    ...
    obj$msg <- paste0(
      obj$msg,
      "\n\nLoaded from the local parsed cache; supplementary files were not scanned again."
    )
    return(obj)
  }
  ...
  obj <- tryCatch({
    ...
    message("Series Matrix loading failed; entering the supplementary fallback. Reason:", conditionMessage(e1))
...
read_metadata_any <- function(f) {
  ...
  if (length(vars) == 0) {
    message("Failed to read metadata: ", basename(f))
    return(NULL)
  }
  vars[[1]]
}
...
find_best_supplementary_metadata <- function(supp_dir, expr_samples, series_meta = NULL) {
  ...
  if (length(meta_files) > 0) {
    for (mf in meta_files) {
      message("Trying supplementary metadata: ", basename(mf))

      variants <- read_metadata_variants(mf)
      if (length(variants) == 0) {
        message("Metadata read failed: ", basename(mf))
        next
      }
      ...
      for (variant_name in names(variants)) {
        ...
        message("Metadata match: ", basename(mf),
        ...
        if (isTRUE(s$high_confidence)) {
          message("High-confidence metadata match found; stopping other skip attempts for this file: ",
...
run_limma_analysis <- function(raw, df2, groupA, groupB, det) {
  ...
  list(
    ...
    message = paste0(
      ...
      "Input classification:", limma_runtime_type, "\n",
      "Input classification reason:", limma_runtime_reason, "\n",
      "limma input transformation:", limma_transform_used
    )
  )
}
...
run_limma_voom_analysis <- function(raw, df2, groupA, groupB, det) {
  ...
  list(
    ...
    message = paste0(
      "limma-voom completed.\n",
      "Contrast: ", groupB, " vs ", groupA, "\n",
      "Samples: ", paste(table(group), collapse = " vs "), "\n",
      "Genes after filtering: ", nrow(v$E), "\n",
      "Input classification:", det$type, "\n",
      "limma input transformation:raw count -> TMM normalization -> voom precision weights"
    )
  )
}
...
run_auto_differential <- function(raw, df2, groupA, groupB, user_method = "auto", det = NULL) {
  ...
  if (method == "DESeq2") {
    if (det$type != "raw_count" && user_method == "DESeq2") {
      warning("DESeq2 was forced, but the input does not look like raw counts. Treat the result as exploratory only.")
    }
    run_deseq2_analysis(raw, df2, groupA, groupB, det)
  } else if (method == "limma_voom") {
    if (det$type != "raw_count") {
      warning("limma-voom requires raw counts; the current input does not look like raw counts. Use ordinary limma instead.")
    }
...
merge_single_sample_expr_files_robust <- function(files, max_files = 5000) {
  ...
  for (f in files) {
    dat <- tryCatch(read_one_single_sample_expr_file_robust(f), error = function(e) {
      message("Per-sample file read failed and was skipped: ", basename(f), " | ", conditionMessage(e))
      NULL
    })
  ...
  message("Per-sample files recognized:", length(count_list), "; failed/skipped:", failed)
  if (length(count_list) > 0) {
    message("Recognized sample examples:", paste(head(names(count_list), 10), collapse = ", "))
  }
  if (length(count_list) < 2) return(NULL)
  ...
  message("Per-sample matrix merge completed:", nrow(raw), " genes x ", length(sample_cols), " samples")
  collapse_by_symbol_auto(raw, forced_method = "sum", source_hint = paste(basename(files), collapse = " "))
}
...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      checkboxInput("clean_cache", "Delete the current GSE download before loading", value = FALSE),
      checkboxInput("use_parsed_cache", "Prefer parsed cache (recommended, faster)", value = TRUE),
      checkboxInput("rebuild_parsed_cache", "Force re-parsing of the current GSE", value = FALSE),
      actionButton("run", "1. Load / Reload GSE"),
      hr(),
      textInput("gene", "Gene symbol (changing the gene does not require rerunning DEG)", value = "CPNE2"),
      hr(),
      selectInput("filter_col", "Filter column (optional pre-filter)", choices = NULL),
      selectInput("filter_value", "Filter value", choices = NULL),
      hr(),
      selectInput("group_col", "Comparison column", choices = NULL),
      selectInput("groupA", "Seed Group A / reference seed (initial choice; may not be the final group after merging)", choices = NULL),
      selectInput("groupB", "Seed Group B / case seed (initial choice; may not be the final group after merging)", choices = NULL),

      hr(),
      checkboxInput("enable_group_merge", "Enable manual group merge (define final analysis groups)", value = FALSE),
      conditionalPanel(
        condition = "input.enable_group_merge == true",
        textInput("mergeA_name", "Final Group A name (used in DEG and plots)", value = "Control"),
        selectizeInput("mergeA_groups", "Metadata levels included in Final Group A", choices = NULL, multiple = TRUE),
        textInput("mergeB_name", "Final Group B name (used in DEG and plots)", value = "Sepsis"),
        selectizeInput("mergeB_groups", "Metadata levels included in Final Group B", choices = NULL, multiple = TRUE),
        helpText("When merging is enabled, DEG, single-gene, volcano, and heatmap modules use Final Group A/B. Seed Group A/B only initializes selections and does not define the final groups. Original metadata is not overwritten.")
      ),

      hr(),
      selectInput(
        "expr_source",
        "Expression source (Auto normally prefers raw data; RPM/TPM/FPKM can be selected manually)",
        choices = c("Auto recommended after Load" = "auto"),
        selected = "auto"
      ),
      helpText("Auto uses the source recommended during loading. If raw and RPM samples differ for a GSE, switch matrices here. The selected matrix is used consistently for DEG, single-gene plots, heatmaps, PCA, and related modules."),
      ...
      hr(),
      helpText("Load reads the data and recommends Seed Groups. If Manual Merge is enabled, confirm the Final Groups and click Run DEG. Single-gene display settings are in the Single-Gene Plot tab."),
      hr(),
      ...
      downloadButton("downloadMergedExpressionMatrix", "Export merged expression matrix CSV"),
      br(), br(),
      downloadButton("downloadExpressionAvailableMetadata", "Export expression-matched groups CSV"),
      br(), br(),
      downloadButton("downloadPlotScaleExpressionMatrix", "Export log/plot-scale matrix CSV"),
      br(), br(),
      downloadButton("downloadMetaReadyDegMatrix", "Export actual Run DEG meta-ready matrix CSV (recommended)"),
      br(), br(),
      helpText("For meta-analysis or WGCNA, use the actual Run DEG meta-ready matrix. It comes directly from the last successful DEG run and preserves the exact samples, groups, and plot-scale expression columns without rematching metadata."),
      br(),
      helpText("Merged expression matrix = the Symbol-by-sample matrix from the current expression source. Raw counts and TPM/FPKM values remain unchanged. The log/plot-scale matrix is used for correlations, heatmaps, PCA, and single-gene displays."),
      hr(),
      helpText("Image download buttons are available within each plotting module.")
    ),
    mainPanel(
      tabsetPanel(
        tabPanel(
        ...
        tabPanel(
              ...
              div(
                ...
                div(
                  ...
                  fluidRow(
                    column(
                      ...
                      selectInput(
                        ...
                        choices = c(
                          ...
                          "Nature / Clean" = "nature",
                          "Minimal" = "minimal",
                          "Current custom settings" = "custom"
                        ),
                        selected = "prism"
                      )
                    ),
                    column(
                      4,
                      actionButton("apply_plot_preset", "Apply style", class = "btn-primary")
                    ),
                    column(
                      4,
                      actionButton("reset_plot_style", "Restore defaults")
                    )
                  )
                ),
                wellPanel(
                  fluidRow(
                    column(
                      ...
                      radioButtons(
                        ...
                        choices = c(
                          "Box plot only" = "box",
                          "Violin plot only" = "violin",
                          "Violin + box plot" = "violin_box"
                        ),
                        selected = "violin_box"
                      ),
                      checkboxInput("show_legend", "Show legend", value = TRUE),
                      checkboxInput("show_major_grid", "Show major grid lines", value = TRUE),
                      checkboxInput("show_minor_grid", "Show minor grid lines", value = FALSE),
                    ...
                    column(
                      6,
                      h4("Colors"),
                      colourpicker::colourInput("groupA_color", "Group A color", value = "#E64B35"),
                      colourpicker::colourInput("groupB_color", "Group B color", value = "#0072B5"),
                      colourpicker::colourInput("other_color", "Other-group color", value = "#00A087"),
                      sliderInput("fill_alpha", "Box/violin fill opacity", min = 0, max = 1, value = 0.22, step = 0.05)
                    )
                  ),
                  ...
                  fluidRow(
                    column(
                      6,
                      h4("Points and group spacing"),
                      sliderInput("point_size", "Point size", min = 1, max = 8, value = 3, step = 0.2),
                      sliderInput("point_alpha", "Point opacity", min = 0.1, max = 1, value = 0.9, step = 0.05),
                      sliderInput("jitter_width", "Horizontal jitter width", min = 0.01, max = 0.5, value = 0.16, step = 0.01),
                      sliderInput("x_spacing", "Visual spacing between groups", min = 0.6, max = 2.5, value = 1.0, step = 0.1),
                      selectInput(
                        "point_shape",
                        "Point shape",
                        choices = c("Filled circle" = 16, "Open circle" = 1, "Square" = 15, "Diamond" = 18, "Triangle" = 17),
                        selected = 16
                      ),
                      sliderInput("point_stroke", "Point border width", min = 0, max = 2, value = 0.5, step = 0.1)
                    ),
                    column(
                      6,
                      h4("Box/violin"),
                      sliderInput("box_line_width", "Box outline width", min = 0.2, max = 3, value = 1.1, step = 0.1),
                      sliderInput("box_width", "Box width", min = 0.15, max = 0.9, value = 0.42, step = 0.05),
                      sliderInput("violin_line_width", "Violin outline width", min = 0.2, max = 3, value = 0.9, step = 0.1),
                      sliderInput("violin_width", "Violin width", min = 0.2, max = 1.2, value = 0.85, step = 0.05)
                    )
                  ),
                  ...
                  fluidRow(
                    column(
                      6,
                      h4("Plot dimensions"),
                      sliderInput("plot_height_px", "Display height (px)", min = 400, max = 1000, value = 650, step = 50),
                      sliderInput("export_width", "Export width (inches)", min = 4, max = 16, value = 10, step = 0.5),
                      sliderInput("export_height", "Export height (inches)", min = 3, max = 12, value = 7, step = 0.5)
                    ),
                    column(
                      6,
                      h4("Typography"),
                      sliderInput("base_font_size", "Base font size", min = 8, max = 24, value = 13, step = 1),
                      sliderInput("axis_text_angle", "X-axis text angle", min = 0, max = 90, value = 45, step = 5),
                      sliderInput("title_font_size", "Title font size", min = 10, max = 30, value = 16, step = 1),
                      sliderInput("subtitle_font_size", "Subtitle font size", min = 7, max = 24, value = 12, step = 1),
                      selectInput(
                        "plot_font_family",
                        "Typography",
                        choices = c("System default" = "", "Arial" = "Arial", "Helvetica" = "Helvetica",
                  ...
                  fluidRow(
                    column(
                      6,
                      h4("Margins"),
                      sliderInput("plot_margin_top", "Top margin", min = 0, max = 40, value = 10, step = 2),
                      sliderInput("plot_margin_right", "Right margin", min = 0, max = 40, value = 10, step = 2),
                      sliderInput("plot_margin_bottom", "Bottom margin", min = 0, max = 40, value = 10, step = 2),
                      sliderInput("plot_margin_left", "Left margin", min = 0, max = 40, value = 10, step = 2)
                    ),
                    column(
                      6,
                      h4("Axes"),
                      checkboxInput("free_y_expand", "Automatic Y-axis padding", value = TRUE),
                      sliderInput("y_expand_ratio", "Y-axis padding ratio", min = 0.01, max = 0.30, value = 0.08, step = 0.01),
                      checkboxInput("manual_y_limits", "Set Y-axis range manually", value = FALSE),
                      conditionalPanel(
                        condition = "input.manual_y_limits == true",
                        fluidRow(
                          column(6, numericInput("y_axis_min", "Y minimum", value = 0)),
                          column(6, numericInput("y_axis_max", "Y maximum", value = 10))
                        )
                      ),
                      numericInput("y_decimal_digits", "Y-axis decimal places", value = 1, min = 0, max = 6, step = 1),
                      checkboxInput("hide_both_axis_titles", "Hide both axis titles", value = FALSE),
                      checkboxInput("hide_x_title", "Hide X-axis title", value = FALSE),
                      checkboxInput("hide_y_title", "Hide Y-axis title", value = FALSE),
                      checkboxInput("hide_x_tick_labels", "Hide X-axis tick labels (group names)", value = FALSE),
                      checkboxInput("hide_y_tick_labels", "Hide Y-axis tick labels (expression values)", value = FALSE)
                    )
                  ),
                  ...
                  fluidRow(
                    column(
                      6,
                      h4("Axis-line width"),
                      sliderInput("x_axis_line_width", "X-axis line width", min = 0, max = 3, value = 0.8, step = 0.1),
                      sliderInput("y_axis_line_width", "Y-axis line width", min = 0, max = 3, value = 0.8, step = 0.1),
                      sliderInput("axis_tick_width", "Tick-line width", min = 0, max = 3, value = 0.8, step = 0.1),
                      sliderInput("axis_tick_length", "Tick length (pt)", min = 0, max = 12, value = 4, step = 0.5)
                    ),
                    column(
                      6,
                      h4("Tick spacing"),
                      textInput("y_major_interval", "Y-axis major interval (blank = automatic)", value = ""),
                      textInput("y_minor_interval", "Y-axis minor interval (blank = automatic/unspecified)", value = ""),
                      helpText("For example, enter 1 or 0.5 for the major interval; the value must be positive.")
                    )
                  ),
                  ...
                  fluidRow(
                    column(
                      6,
                      h4("Axis Typography"),
                      sliderInput("axis_text_size", "Tick-label size", min = 6, max = 24, value = 11, step = 1),
                      sliderInput("axis_title_size", "Axis-title size", min = 6, max = 26, value = 13, step = 1),
                      checkboxInput("bold_axis_text", "Bold tick labels", value = FALSE),
                      checkboxInput("bold_axis_title", "Bold axis titles", value = FALSE)
                    ),
                    column(
                      6,
                      h4("Tick display"),
                      checkboxInput("show_x_ticks", "Show X-axis ticks", value = TRUE),
                      checkboxInput("show_y_ticks", "Show Y-axis ticks", value = TRUE),
                      checkboxInput("show_x_axis_line", "Show X-axis line", value = TRUE),
                      checkboxInput("show_y_axis_line", "Show Y-axis line", value = TRUE)
                    )
                  ),
                  ...
                  fluidRow(
                    column(
                      6,
                      h4("Titles and legend"),
                      textInput("custom_plot_title", "Custom title (blank = automatic)", value = ""),
                      textInput("custom_plot_subtitle", "Custom subtitle (blank = automatic)", value = ""),
                      textInput("custom_x_title", "Custom X-axis title (blank = Group)", value = ""),
                      textInput("custom_y_title", "Custom Y-axis title (blank = automatic expression label)", value = ""),
                      selectInput(
                        "legend_position",
                        "Legend position",
                      ...
                      sliderInput("legend_text_size", "Legend text size", min = 6, max = 22, value = 11, step = 1),
                      sliderInput("legend_title_size", "Legend title size", min = 6, max = 24, value = 12, step = 1)
                    ),
                    column(
                      6,
                      h4("Statistical annotation and means"),
                      selectInput(
                        "plot_p_label_type",
                        "P-value display",
                        choices = c("Exact P-value" = "p.format", "Significance stars" = "p.signif"),
                        selected = "p.format"
                      ),
                      sliderInput("plot_p_text_size", "P-value text size", min = 2, max = 10, value = 4.5, step = 0.5),
                      textInput("plot_p_y", "P-value height (blank = automatic)", value = ""),
                      checkboxInput("show_mean_marker", "Show group-mean markers", value = FALSE),
                      sliderInput("mean_marker_size", "Mean-marker size", min = 1, max = 10, value = 4, step = 0.5),
                      selectInput(
                        "mean_marker_shape",
                        "Mean-marker shape",
                        choices = c("Diamond" = 18, "Filled circle" = 16, "Square" = 15, "Triangle" = 17),
                        selected = 18
                      ),
                      colourpicker::colourInput("mean_marker_color", "Mean-marker color", value = "#000000")
                    )
                  )
                )
              )
          ...
          wellPanel(
            h4("Image Export"),
            fluidRow(
              column(3, downloadButton("downloadPlotPNG", "Download single-gene plot PNG")),
              column(3, downloadButton("downloadPlotPDF", "Download single-gene plot PDF")),
              column(3, downloadButton("downloadPlotSVG", "Download single-gene plot SVG"))
            )
          ),

          wellPanel(
            h4("Single-Gene Statistics"),
            ...
            fluidRow(
              column(
                12,
                h4("Two-Group Comparison Statistics"),
                uiOutput("single_gene_comparison_cards")
              )
            ),

            br(),
            downloadButton("downloadSingleGeneStats", "Export single-gene statistics CSV")
          )
        ),
          ...
          wellPanel(
            fluidRow(
              column(
                3,
                h4("Thresholds"),
                sliderInput("volcano_logfc_cutoff", "log2FC threshold", min = 0, max = 3, value = 1, step = 0.1),
                sliderInput("volcano_padj_cutoff", "Adjusted-P threshold", min = 0.001, max = 0.2, value = 0.05, step = 0.001),
                radioButtons(
                  "volcano_p_type",
                  "Y-axis metric",
              ...
              column(
                3,
                h4("Colors"),
                colourpicker::colourInput("volcano_up_color", "Upregulated color", value = "#E64B35"),
                colourpicker::colourInput("volcano_down_color", "Downregulated color", value = "#0072B5"),
                colourpicker::colourInput("volcano_ns_color", "Not-significant color", value = "#BDBDBD"),
                colourpicker::colourInput("volcano_target_color", "Target-gene color", value = "#000000")
              ),
              column(
                3,
                h4("Points and labels"),
                sliderInput("volcano_point_size", "Point size", min = 0.3, max = 4, value = 1.2, step = 0.1),
                sliderInput("volcano_point_alpha", "Point opacity", min = 0.1, max = 1, value = 0.7, step = 0.05),
                sliderInput("volcano_target_size", "Target-gene point size", min = 1, max = 8, value = 3.5, step = 0.2),
                checkboxInput("volcano_show_target_label", "Show target-gene label", value = TRUE)
              ),
              column(
                3,
                h4("Display"),
                checkboxInput("volcano_show_cutoff_lines", "Show threshold lines", value = TRUE),
                checkboxInput("volcano_show_legend", "Show legend", value = TRUE),
                sliderInput("volcano_base_font_size", "Font size", min = 8, max = 24, value = 13, step = 1),
                sliderInput("volcano_height_px", "Display height (px)", min = 400, max = 1000, value = 650, step = 50)
              )
            ),
            hr(),
            fluidRow(
              column(3, sliderInput("volcano_export_width", "Export width (inches)", min = 4, max = 16, value = 10, step = 0.5)),
              column(3, sliderInput("volcano_export_height", "Export height (inches)", min = 3, max = 12, value = 7, step = 0.5)),
              column(3, selectInput("volcano_export_dpi", "Export DPI", choices = c(300, 600, 1200), selected = 600)),
              column(3, br(), checkboxInput("volcano_export_label", "Show target label in exports", value = TRUE))
            )
          ),
          wellPanel(
            h4("Volcano Plot Export"),
            fluidRow(
              column(3, downloadButton("downloadVolcanoPNG", "Download volcano plot PNG")),
              column(3, downloadButton("downloadVolcanoPDF", "Download volcano plot PDF")),
              column(3, downloadButton("downloadVolcanoSVG", "Download volcano plot SVG"))
            ),
            br(),
            fluidRow(
              column(3, downloadButton("downloadUpGenes", "Export upregulated genes CSV")),
              column(3, downloadButton("downloadDownGenes", "Export downregulated genes CSV")),
              column(3, downloadButton("downloadVolcanoSigGenes", "Export significant genes CSV"))
            )
          ),
          uiOutput("volcano_ui")
        ),
        ...
        tabPanel(
          "Clustered Heatmap",
          h4("Clustered Heatmap Controls"),
          wellPanel(
            fluidRow(
              column(
                3,
                h4("Gene filtering"),
                numericInput("heatmap_top_n", "Number of top genes", value = 50, min = 5, max = 500, step = 5),
                sliderInput("heatmap_logfc_cutoff", "Minimum |log2FC|", min = 0, max = 3, value = 0, step = 0.1),
                sliderInput("heatmap_padj_cutoff", "Maximum adjusted P", min = 0.001, max = 1, value = 0.05, step = 0.001),
                radioButtons(
                  "heatmap_rank_by",
                  "Sort by",
                  choices = c("Smallest adjusted P" = "padj", "Largest |log2FC|" = "abslogfc", "Smallest P-value" = "pvalue"),
                  selected = "padj"
                )
              ),
              column(
                3,
                h4("Clustering"),
                checkboxInput("heatmap_cluster_rows", "Cluster genes", value = TRUE),
                checkboxInput("heatmap_cluster_cols", "Cluster samples", value = TRUE),
                checkboxInput("heatmap_show_rownames", "Show gene names", value = TRUE),
                checkboxInput("heatmap_show_colnames", "Show sample names", value = FALSE),
                radioButtons(
                  "heatmap_scale",
                  "Data scaling",
                  choices = c("Gene-wise Z-score" = "row", "No scaling" = "none"),
                  selected = "row"
                )
              ),
              column(
                ...
                selectInput(
                  "heatmap_palette",
                  "Heatmap palette",
                  choices = c("Blue-White-Red" = "blue_white_red", "Green-Black-Red" = "green_black_red", "Purple-White-Orange" = "purple_white_orange"),
                  selected = "blue_white_red"
                ),
                colourpicker::colourInput("heatmap_groupA_color", "Group A annotation color", value = "#E64B35"),
                colourpicker::colourInput("heatmap_groupB_color", "Group B annotation color", value = "#0072B5"),
                sliderInput("heatmap_fontsize", "Overall font size", min = 5, max = 18, value = 9, step = 1),
                sliderInput("heatmap_fontsize_row", "Gene-name font size", min = 3, max = 16, value = 7, step = 1)
              ),
              column(
                3,
                h4("Dimensions"),
                sliderInput("heatmap_height_px", "Display height (px)", min = 400, max = 1200, value = 700, step = 50),
                sliderInput("heatmap_cellwidth", "Cell width", min = 4, max = 30, value = 10, step = 1),
                sliderInput("heatmap_cellheight", "Cell height", min = 4, max = 30, value = 10, step = 1),
                sliderInput("heatmap_border_width", "Cell-border width", min = 0, max = 1, value = 0, step = 0.1)
              )
            ),
            hr(),
            fluidRow(
              column(3, sliderInput("heatmap_export_width", "Export width (inches)", min = 4, max = 20, value = 10, step = 0.5)),
              column(3, sliderInput("heatmap_export_height", "Export height (inches)", min = 4, max = 20, value = 9, step = 0.5)),
              column(3, selectInput("heatmap_export_dpi", "Export DPI", choices = c(300, 600, 1200), selected = 600))
            )
          ),
          wellPanel(
            h4("Heatmap Export"),
            fluidRow(
              column(3, downloadButton("downloadHeatmapPNG", "Download heatmap PNG")),
              column(3, downloadButton("downloadHeatmapPDF", "Download heatmap PDF")),
              column(3, downloadButton("downloadHeatmapSVG", "Download heatmap SVG")),
              column(3, downloadButton("downloadHeatmapGenes", "Export heatmap genes CSV"))
            )
          ),
          uiOutput("heatmap_ui")
        ),
        ...
        tabPanel(
          ...
          wellPanel(
            fluidRow(
              column(
                3,
                numericInput("pca_top_var_genes", "Number of highly variable genes for PCA", value = 1000, min = 100, max = 10000, step = 100),
                sliderInput("pca_point_size", "Point size", min = 1, max = 8, value = 3.5, step = 0.2),
                sliderInput("pca_point_alpha", "Point opacity", min = 0.1, max = 1, value = 0.85, step = 0.05)
              ),
              column(
                3,
                colourpicker::colourInput("pca_groupA_color", "Group A color", value = "#E64B35"),
                colourpicker::colourInput("pca_groupB_color", "Group B color", value = "#0072B5"),
                colourpicker::colourInput("pca_other_color", "Other-group color", value = "#00A087")
              ),
              column(
                3,
                checkboxInput("pca_show_labels", "Show sample names", value = FALSE),
                checkboxInput("pca_show_ellipse", "Show group ellipses", value = TRUE),
                checkboxInput("pca_show_grid", "Show grid lines", value = TRUE)
              ),
              column(
                3,
                sliderInput("pca_base_font_size", "Font size", min = 8, max = 24, value = 13, step = 1),
                sliderInput("pca_height_px", "Display height (px)", min = 400, max = 1000, value = 650, step = 50),
                sliderInput("pca_export_width", "Export width (inches)", min = 4, max = 16, value = 8, step = 0.5),
                sliderInput("pca_export_height", "Export height (inches)", min = 3, max = 12, value = 6, step = 0.5)
              )
            )
          ),
          wellPanel(
            h4("PCA Export"),
            fluidRow(
              column(3, downloadButton("downloadPCAPNG", "Download PCA PNG")),
              column(3, downloadButton("downloadPCAPDF", "Download PCA PDF")),
              column(3, downloadButton("downloadPCASVG", "Download PCA SVG"))
            )
          ),
          uiOutput("pca_ui")
        ),
        ...
        tabPanel(
          ...
          tabsetPanel(
            tabPanel(
              ...
              wellPanel(
                fluidRow(
                  ...
                  column(3, selectInput("corr_method", "Correlation method", choices = c("Spearman" = "spearman", "Pearson" = "pearson"), selected = "spearman")),
                  column(3, checkboxInput("corr_use_selected_groups", "Use current A/B groups only", value = TRUE))
                ),
                fluidRow(
                  column(3, sliderInput("corr_point_size", "Point size", min = 1, max = 8, value = 3, step = 0.2)),
                  column(3, sliderInput("corr_point_alpha", "Point opacity", min = 0.1, max = 1, value = 0.85, step = 0.05)),
                  column(3, checkboxInput("corr_show_lm", "Show linear fit", value = TRUE)),
                  column(3, checkboxInput("corr_show_labels", "Show sample names", value = FALSE))
                )
              ),
            ...
            tabPanel(
              ...
              wellPanel(
                fluidRow(
                  ...
                  column(3, numericInput("corr_all_top_n", "Number of top results", value = 30, min = 5, max = 200, step = 5)),
                  column(3, selectInput("corr_all_method", "Correlation method", choices = c("Spearman" = "spearman", "Pearson" = "pearson"), selected = "spearman")),
                  column(3, checkboxInput("corr_all_current_groups", "Use current A/B groups only", value = TRUE))
                )
              ),
              fluidRow(
              ...
              downloadButton("downloadCorrAll", "Export all-gene correlations CSV")
            ),
            ...
            tabPanel(
              ...
              wellPanel(
                fluidRow(
                  column(6, textAreaInput("corr_family_genes", "Gene list (comma- or newline-separated)", value = "CPNE1, CPNE2, CPNE3, CPNE4, CPNE5, CPNE6, CPNE7, CPNE8, CPNE9", rows = 4)),
                  column(3, selectInput("corr_family_method", "Correlation method", choices = c("Spearman" = "spearman", "Pearson" = "pearson"), selected = "spearman")),
                  column(3, checkboxInput("corr_family_cluster", "Cluster genes", value = TRUE))
                )
              ),
              plotOutput("corr_family_heatmap", height = "700px"),
              downloadButton("downloadCorrFamily", "Export correlation matrix CSV")
            )
          )
        ),
        ...
        tabPanel(
          "GO/KEGG",
          h4("Functional Enrichment Analysis"),
          wellPanel(
            fluidRow(
              column(3, selectInput("enrich_direction", "Gene set", choices = c("All DEGs" = "all", "Up genes" = "up", "Down genes" = "down"), selected = "all")),
              column(3, sliderInput("enrich_padj_cutoff", "DEG adjusted-P threshold", min = 0.001, max = 1, value = 0.05, step = 0.001)),
              column(3, sliderInput("enrich_logfc_cutoff", "DEG |log2FC| threshold", min = 0, max = 3, value = 1, step = 0.1)),
              column(3, numericInput("enrich_show_n", "Number of top terms", value = 15, min = 5, max = 50, step = 5))
            ),
            fluidRow(
              column(3, sliderInput("enrich_p_cutoff", "Enrichment P-value cutoff", min = 0.001, max = 1, value = 0.05, step = 0.001)),
              column(3, sliderInput("enrich_q_cutoff", "Enrichment Q-value cutoff", min = 0.001, max = 1, value = 0.2, step = 0.001)),
              column(3, selectInput("enrich_go_ont", "GO ontology", choices = c("BP", "CC", "MF"), selected = "BP")),
              column(3, actionButton("run_enrichment", "Run GO/KEGG Enrichment"))
            )
          ),

          h4("Genes Used for Enrichment"),
          ...
          br(),
          downloadButton("downloadGO", "Export GO results CSV"),
          downloadButton("downloadKEGG", "Export KEGG results CSV")
        ),
        ...
        tabPanel(
          ...
          helpText("Automatically scans all metadata columns suitable for grouping. Informative disease, condition, phenotype, and subtype columns are preferred; Auto_Group is only a fallback."),
          tableOutput("group_column_overview_table"),
          downloadButton("downloadGroupColumnOverview", "Export group-column overview"),

          hr(),
          h4("All group levels and sample counts"),
          tableOutput("group_backup_summary_table"),
          downloadButton("downloadGroupBackupSummary", "Export all group counts"),

          hr(),
          h4("Metadata preview"),
          tableOutput("meta_preview"),
          downloadButton("downloadFullMetadata", "Export full metadata")
        )
      )
    )
  )
...
server <- function(input, output, session) {
  ...
  run_detail <- reactiveVal("Waiting for GEO data")
  ...
  output$run_status_ui <- renderUI({
    ...
    tags$div(
      ...
      tags$div(class = "run-status-detail", paste0(run_detail(), "  |  Elapsed: ", elapsed)),
      tags$div(
  ...
  observeEvent(input$run, {
    ...
    set_run_status("Reset", paste0("Previous results cleared; preparing to load ", gse_id), 3, "running")

    if (isTRUE(input$clean_cache)) {
      set_run_status("Clear cache", "Deleting current GSE downloads and temporary files", 8, "running")
    ...
    set_run_status("Download/read GEO", "Reading cache, Series Matrix, or supplementary expression matrix", 18, "running")
    ...
    expr_res <- tryCatch({
      ...
      set_run_status("GEO read failed", paste0("Error: ", e$message), 100, "error")
      data_obj(list(error = paste0("GEO read failed: ", e$message)))
      return(NULL)
    })
    if (is.null(expr_res)) return()

    set_run_status("Prepare expression matrix", "Identifying gene names, expression matrix, and data type", 45, "running")
    ...
    set_run_status("Match samples and groups", "Matching expression samples to metadata and detecting groups", 65, "running")
    ...
    set_run_status("Scan expression sources", "Building the raw/RPM/TPM/FPKM candidate matrix list", 88, "running")
    ...
    set_run_status("Loading complete", "Data loaded and Seed Groups recommended. Confirm the expression source and Final Groups, then click Run DEG.", 95, "running")
    ...
    set_run_status("Loading complete", paste0(gse_id, " loaded. Confirm the groups, then click Run DEG."), 100, "done")
  })
  ...
  observeEvent(list(input$groupA, input$groupB, input$method_mode, input$expr_source,
    ...
    diff_obj(NULL)
    deg_has_run(FALSE)
    set_run_status("Waiting for Run DEG", "Groups, filters, expression source, or analysis method changed. Confirm the settings and click Run DEG.", 100, "done")
  }, ignoreInit = TRUE)
  ...
  observeEvent(list(input$enable_group_merge, input$mergeA_groups, input$mergeB_groups, input$mergeA_name, input$mergeB_name), {
    diff_obj(NULL)
    deg_has_run(FALSE)
    set_run_status("Waiting for Run DEG", "Manual group-merge settings changed. Confirm them and click Run DEG.", 100, "done")
  }, ignoreInit = TRUE)
  ...
  observeEvent(input$run_deg, {
    ...
    if (is.null(meta2) || nrow(meta2) == 0) {
      set_run_status("Differential analysis not run", "No samples are available for differential analysis. Check sample matching and groups.", 100, "error")
      diff_obj(list(error = "No samples are available for differential analysis. Check sample matching and groups."))
      return()
    }
    ...
    if (FALSE && !is.null(input$method_mode) && input$method_mode %in% c("limma", "DESeq2")) {
      set_run_status("Select best data source", paste0("Force ", input$method_mode, ": evaluating candidate matrices by current-group coverage"), 88, "running")
      match_mode2 <- ifelse(is.null(input$sample_match_mode), "auto_robust", input$sample_match_mode)
    ...
    if (length(unique(df2$group)) < 2) {
      set_run_status("Differential analysis not run", "Differential analysis requires two groups.", 100, "error")
      diff_obj(list(error = "Differential analysis requires two groups."))
      return()
    }
    ...
    if (length(matched_samples) < 4) {
      set_run_status("Insufficient sample matches", "Too few matched samples to run differential analysis.", 100, "error")
      diff_obj(list(error = "Too few matched samples to run differential analysis."))
      return()
    }
    ...
    if (any(as.integer(group_counts) < 2)) {
      ...
      msg <- paste0(
        "The selected groups do not have enough samples in the expression matrix to run DEG.",
        " At least two expression samples are required per group. Expression-available counts: ", msg_counts,
        ". Choose different Group A/B values or verify that the supplementary matrix contains these groups."
      )
      set_run_status("Insufficient sample count", msg, 100, "error")
      diff_obj(list(
        error = msg,
    ...
    set_run_status("Running differential analysis", paste0("Analyzing: ", groupB_run, " vs ", groupA_run), 92, "running")
    ...
    tryCatch({
      ...
      set_run_status(
        "Analysis complete",
        paste0("Completed ", res$method, " analysis", if (!is.na(elapsed)) paste0("; elapsed ", elapsed, " seconds") else ""),
        100,
        "done"
      )
    }, error = function(e) {
      set_run_status("Differential analysis failed", paste0("Error: ", e$message), 100, "error")
      diff_obj(list(error = paste0("Differential analysis failed: ", e$message)))
      deg_has_run(FALSE)
    })
  })
  ...
  message_text <- reactive({
    ...
    if (is.null(obj)) {
      return("Click 1. Load / Reload GSE. Loading only recommends Seed Groups; if Manual Merge is enabled, confirm the Final Groups and click Run DEG.")
    }
    ...
    if (!is.null(dobj)) {
      ...
      if (!is.null(dobj$error)) {
        ...
        out <- add(out, paste0(dobj$method, " completed."))
        ...
        if (!is.null(dobj$expr_detect_used)) out <- add(out, "Input classification:", dobj$expr_detect_used)
        if (identical(dobj$method, "limma")) {
          if (!is.null(dobj$limma_transform_used)) {
            out <- add(out, "limma input transformation:", dobj$limma_transform_used)
          } else {
            out <- add(out, "limma input transformation:selected automatically from the expression-matrix type")
          }
        }
  ...
  output$single_gene_comparison_cards <- renderUI({
    ...
    if (is.null(st) || nrow(st) == 0) {
      return(tags$div("Two-group comparison statistics are not available yet."))
    }
  ...
  output$corr_family_heatmap <- renderPlot({
    ...
    if (is.null(cm)) {
      plot.new()
      text(0.5, 0.5, "Correlation matrix unavailable: check gene names and sample count.", cex = 1.1)
      return()
    }
  ...
  observeEvent(input$run_enrichment, {
    ...
    if (length(genes) < 3) {
      enrich_obj(list(error = "Fewer than three genes are available for enrichment. Relax the adjusted-P/log2FC thresholds.", genes = genes))
      return()
    }
  ...
  output$enrich_gene_count <- renderText({
    ...
    paste0("Genes available for enrichment at the current thresholds:", length(genes))
  })
  ...
  output$go_dotplot <- renderPlot({
    ...
    if (is.null(eo)) {
      plot.new()
      text(0.5, 0.5, "Click Run GO/KEGG Enrichment to display results.", cex = 1.1)
      return()
    }
    ...
    if (is.null(eo$go) || nrow(as.data.frame(eo$go)) == 0) {
      plot.new()
      text(0.5, 0.5, "No significant GO enrichment results.", cex = 1.1)
      return()
    }
  ...
  output$kegg_dotplot <- renderPlot({
    ...
    if (is.null(eo$kegg) || nrow(as.data.frame(eo$kegg)) == 0) {
      plot.new()
      text(0.5, 0.5, "No significant KEGG enrichment results.", cex = 1.1)
      return()
    }
  ...
  output$pca_plot <- renderPlot({
    ...
    if (is.null(p)) {
      plot.new()
      text(0.5, 0.5, "Insufficient data for PCA, or differential analysis has not completed.", cex = 1.1)
    } else {
      print(p)
    }
  })
  ...
  draw_heatmap <- function(silent = FALSE) {
    ...
    if (is.null(hm)) {
      plot.new()
      text(0.5, 0.5, "Not enough genes for the heatmap. Relax the adjusted-P/log2FC thresholds or increase the number of top genes.", cex = 1.1)
      return(NULL)
    }
